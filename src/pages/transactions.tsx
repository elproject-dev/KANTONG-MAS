...
function TransactionReceiptDialog({
  ...
  return (
    <Dialog open={!!trx} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-lg p-0 overflow-hidden bg-slate-50">
        {!trx ? (
            ...
            <div className="p-4 sm:p-6 overflow-y-auto bg-white m-4 rounded-xl shadow-sm border border-slate-200 printable-receipt [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              ...
              <div className="space-y-3 mb-4 sm:mb-6">
                ...
                <div className="flex justify-between text-xs sm:text-sm">
                  ...
                  <span className="font-medium text-right">{trx.customers?.phone || trx.customerPhone || "-"}</span>
                </div>
                <div className="flex justify-between text-xs sm:text-sm">
                  <span className="text-slate-500">Alamat</span>
                  <span className="font-medium text-right max-w-[60%] truncate" title={trx.customers?.address || "-"}>{trx.customers?.address || "-"}</span>
                </div>
                <div className="flex justify-between text-xs sm:text-sm">
                  <span className="text-slate-500">Kecamatan</span>
                  <span className="font-medium text-right">{trx.customers?.district || "-"}</span>
                </div>
                <div className="flex justify-between text-xs sm:text-sm">
                  <span className="text-slate-500">Kabupaten/Kota</span>
                  <span className="font-medium text-right">{trx.customers?.city || "-"}</span>
                </div>
                <div className="flex justify-between text-xs sm:text-sm">
...
export default function TransactionsPage() {
  ...
  const getPaymentStatusBadge = (trx: any) => {
    if (trx.status === 'cancelled') return <Badge className="bg-yellow-500 hover:bg-yellow-600 text-white font-medium">BATAL</Badge>;
    if (trx.payment_status === 'partial') return <Badge className="bg-amber-500 hover:bg-amber-600 text-white font-medium">CICILAN</Badge>;
    if (trx.payment_status === 'unpaid') return <Badge variant="destructive" className="font-medium">TEMPO</Badge>;
