...
export default function ReceivablesPage() {
  ...
  const storeInfo = {
    ...
    phone: storeSettingsData?.phone || '081234567890',
  ...
  useEffect(() => {
    if (!isAdmin && cashierName) {
      setSalesFilter(cashierName);
      setTempSalesFilter(cashierName);
    }
  }, [isAdmin, cashierName]);
  ...
  const getStatusBadge = (trx: any) => {
    if (trx.status === 'cancelled') return <Badge className="bg-yellow-500 hover:bg-yellow-600 text-white">Batal</Badge>;
    if (trx.payment_status === 'partial') return <Badge className="bg-amber-500 hover:bg-amber-600">Cicilan</Badge>;
    if (trx.payment_status === 'unpaid') return <Badge variant="destructive">Tempo Penuh</Badge>;
    ...
    <Sidebar>
      <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-slate-950">
                        ...
                        paginatedReceivables?.map((trx: any) => {
                          ...
                          return (
                            <TableRow key={trx.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer" onClick={() => handleRowClick(trx)}>
                              ...
                              <TableCell className="text-center whitespace-nowrap">{getStatusBadge(trx)}</TableCell>
        ...
        <Dialog open={isDetailModalOpen} onOpenChange={setIsDetailModalOpen}>
          <DialogContent className="max-w-lg p-0 overflow-hidden bg-slate-50 border-none rounded-xl shadow-2xl scrollbar-hide">
            {!detailTransaction ? (
                ...
                <div className="p-4 sm:p-6 overflow-y-auto bg-white m-4 rounded-xl shadow-sm border border-slate-200 printable-receipt [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                  ...
                  <div className="space-y-3 mb-4 sm:mb-6">
                    <div className="flex justify-between items-center">
                      ...
                      {detailTransaction.status === 'cancelled' ? (
                        <Badge className="bg-yellow-500 hover:bg-yellow-600 text-white font-medium">BATAL</Badge>
                    ...
                    {detailTransaction.customers || detailTransaction.customer ? (
                      <>
                        <div className="flex justify-between text-xs sm:text-sm">
                          <span className="text-slate-500">No. Telp</span>
                          <span className="font-medium text-right">{detailTransaction.customers?.phone || detailTransaction.customer?.phone || detailTransaction.customer_phone || "-"}</span>
                        </div>
                        <div className="flex justify-between text-xs sm:text-sm">
                          <span className="text-slate-500">Alamat</span>
                          <span className="font-medium text-right max-w-[60%] truncate" title={detailTransaction.customers?.address || detailTransaction.customer?.address || detailTransaction.customer_address || "-"}>{detailTransaction.customers?.address || detailTransaction.customer?.address || detailTransaction.customer_address || "-"}</span>
                        </div>
                        <div className="flex justify-between text-xs sm:text-sm">
                          <span className="text-slate-500">Kecamatan</span>
                          <span className="font-medium text-right">{detailTransaction.customers?.district || detailTransaction.customer?.district || detailTransaction.customer_district || "-"}</span>
                        </div>
                        <div className="flex justify-between text-xs sm:text-sm">
                          <span className="text-slate-500">Kabupaten/Kota</span>
                          <span className="font-medium text-right">{detailTransaction.customers?.city || detailTransaction.customer?.city || detailTransaction.customer_city || "-"}</span>
                        </div>
                      </>
                    ) : null}
                    <div className="flex justify-between text-xs sm:text-sm">
