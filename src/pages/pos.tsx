                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="relative">
                    <button
                      type="button"
                      onClick={() => {
                        const el = document.getElementById('pos-due-date-input') as HTMLInputElement;
                        if (el) { el.focus(); el.showPicker?.(); }
                      }}
                      className="w-full h-10 px-3 text-left flex items-center justify-between border border-slate-200 dark:border-slate-800 rounded-md bg-white dark:bg-slate-900 cursor-pointer text-sm font-medium hover:border-slate-400 transition-colors"
                    >
                      <span className={dueDate ? "text-slate-900 dark:text-slate-100" : "text-slate-400"}>
                        {dueDate
