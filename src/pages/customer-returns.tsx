...
export default function CustomerReturnsPage() {
  ...
  const handleFullReturn = () => {
    if (!transaction) return;
    if (!confirm(`Apakah Anda yakin ingin melakukan RETUR PENUH (Batal Transaksi)?\nSemua barang akan dikembalikan ke stok dan sisa tagihan (jika ada) akan dihapuskan.`)) return;

    const itemsToReturn = transaction.items.map((item: any) => {
      const selectedUnit = { unit_name: item.unit_name || 'PCS', conversion_factor: item.conversion_factor || 1 };

      const maxBaseQty = (item.quantity * (item.conversion_factor || 1)) - (item.already_returned_qty || 0);
      const maxQtyInUnit = Math.floor(maxBaseQty / selectedUnit.conversion_factor);

      const basePrice = Number(item.price);
      const refundPrice = basePrice * selectedUnit.conversion_factor;
      return {
        ...item,
        return_unit_name: selectedUnit.unit_name,
        return_conversion_factor: selectedUnit.conversion_factor,
        return_quantity: maxQtyInUnit,
        return_price: refundPrice,
        return_subtotal: maxQtyInUnit * refundPrice
      };
    }).filter((item: any) => item.return_quantity > 0);

    if (itemsToReturn.length === 0) {
      toast({ title: "Peringatan", description: "Semua barang di transaksi ini sudah diretur sebelumnya.", variant: "destructive" });
      return;
    }

    const fullRefundAmount = itemsToReturn.reduce((sum: number, item: any) => sum + item.return_subtotal, 0);

    createReturn.mutate({
      transactionId: transaction.id,
      customerId: transaction.customer_id,
      cashierName: cashierName,
      totalRefund: fullRefundAmount,
      reason: "Retur Penuh / Batal Transaksi",
      notes: "Pembatalan seluruh transaksi (Otomatis)",
      items: itemsToReturn,
      status: isAdmin ? 'completed' : 'pending'
    }, {
      onSuccess: () => {
        toast({ title: "Sukses", description: "Retur penuh (Batal Transaksi) berhasil diproses.", variant: "success" });
        setSearchedId(null);
        setSearchInvoice("");
        setActiveTab(isAdmin ? "history" : "pending");
      },
                    ...
                    <Card className="shadow-sm border-slate-200 dark:border-slate-800">
                      ...
                      <CardContent className="p-6 space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                          <Button
                            className="w-full sm:flex-1 gap-2 h-14 text-base font-bold bg-rose-100 text-rose-700 hover:bg-rose-200 hover:text-rose-800 shadow-sm transition-all"
                            size="lg"
                            variant="secondary"
                          ...
                          <Button
                            className="w-full sm:flex-1 gap-2 h-14 text-base font-bold transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:pointer-events-none disabled:transform-none disabled:shadow-none"
                            size="lg"
                            onClick={handleSubmitReturn}
                            disabled={totalRefundItems === 0 || !reason || createReturn.isPending}
                          >
                            <CheckCircle2 className="w-6 h-6" />
                            {createReturn.isPending ? "Memproses Retur..." : "Retur Sebagian"}
                          </Button>
                        </div>
                      </CardContent>
                    </Card>
