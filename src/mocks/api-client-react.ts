  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      if (allNeededTxIds.length > 0) {
        const { data: txs } = await applyTenantFilter(
          supabase
            .from('transactions')
      ...
      const totalRevenuePeriod = Math.max(0,
      ...
      const totalRevenueToday = Math.max(0,
...
export const useDeleteTransaction = () => {
  ...
  return {
    mutate: async (params: { id: number }, options?: any) => {
      setIsPending(true);
      try {
        // Step 0: Check if transaction is already cancelled
        const { data: trx } = await applyTenantFilter(
          supabase.from('transactions').select('status').eq('id', params.id).single()
        );

        // Hanya kembalikan stok jika transaksi belum dibatalkan (retur penuh)
        if (trx?.status !== 'cancelled') {
          // Step 1: Fetch transaction items and their related return items
          const { data: items } = await applyTenantFilter(
            supabase
              .from('transaction_items')
              .select('*, products(product_uoms(*)), sales_return_items(quantity, unit_name)')
              .eq('transaction_id', params.id)
          );

          if (items && items.length > 0) {
            // Step 2 & 3: Loop items and restore stock
            for (const item of items) {
              const uoms = item.products?.product_uoms || [];
              const uom = uoms.find((u: any) => u.unit_name === item.unit_name);
              const conversionFactor = uom ? uom.conversion_factor : (item.conversion_factor || 1);
              
              const qty = item.unit_qty !== undefined && item.unit_qty !== null ? item.unit_qty : item.quantity;
              let totalPcsToRestore = qty * conversionFactor;

              // Kurangi dengan stok yang sudah pernah diretur (agar tidak double tambah stok)
              if (item.sales_return_items && item.sales_return_items.length > 0) {
                let totalReturnedPcs = 0;
                item.sales_return_items.forEach((retItem: any) => {
                  const retUom = uoms.find((u: any) => u.unit_name === retItem.unit_name);
                  const retFactor = retUom ? retUom.conversion_factor : 1;
                  totalReturnedPcs += (retItem.quantity || 0) * retFactor;
                });
                totalPcsToRestore -= totalReturnedPcs;
              }

              if (totalPcsToRestore > 0) {
                const { data: product } = await supabase.from('products').select('stock_quantity').eq('id', item.product_id).single();
                if (product && product.stock_quantity !== null) {
                  const newStock = product.stock_quantity + totalPcsToRestore;
                  ...
                  await supabase.from('stock_movements').insert({
                    ...
                    quantity: totalPcsToRestore,
                    type: 'adjustment',
                    reference_id: params.id.toString(),
                    note: `Pengembalian stok dari transaksi dihapus (INV-${params.id})`
                  });
                }
              }
            }
        ...
        // Step 5: Delete items and transaction
    ...
    mutate: async (params: { transactionId: number; customerId: number | null; cashierName: string; totalRefund: number; reason: string; notes?: string; items: any[]; status?: string }, options?: any) => {
      ...
      try {
        ...
        if (isCompleted) {
          ...
          // Transaction cancellation logic
          const isBatal = params.reason.includes('Batal Transaksi') || params.reason.includes('Retur Penuh');
          const { data: trx } = await supabase.from('transactions').select('payment_status, remaining_balance, owner_id').eq('id', params.transactionId).single();
          
          if (trx) {
            let updatePayload: any = {};
            let debtCancelled = 0;

            if ((trx.payment_status === 'unpaid' || trx.payment_status === 'partial') && trx.remaining_balance > 0) {
              let newBalance = trx.remaining_balance - params.totalRefund;
              if (isBatal) {
                 newBalance = 0; // Wipe it completely
              } else {
                 newBalance = Math.max(0, newBalance);
              }
              debtCancelled = trx.remaining_balance - newBalance;
              updatePayload.remaining_balance = newBalance;
              updatePayload.payment_status = newBalance <= 0 ? 'paid' : trx.payment_status;
            }
            ...
            if (Object.keys(updatePayload).length > 0) {
              await supabase.from('transactions').update(updatePayload).eq('id', params.transactionId);
            }

            if (debtCancelled > 0) {
              await supabase.from('transaction_payments').insert({
                transaction_id: params.transactionId,
                amount: debtCancelled,
                payment_method: 'Potongan Retur',
                cashier_name: params.cashierName,
                notes: 'Otomatis dari proses retur',
                payment_date: new Date().toISOString(),
                status: 'confirmed',
                owner_id: trx.owner_id
              });
            }
          }
        }
  ...
  return {
    mutate: async (params: { returnId: number }, options?: any) => {
      ...
      try {
        ...
        // Fetch return reason and details
        const { data: salesReturn, error: returnError } = await supabase
          .from('sales_returns')
          .select('reason, transaction_id, total_refund, cashier_name')
          .eq('id', params.returnId)
          .single();
        ...
        // Transaction cancellation logic for confirm
        if (salesReturn?.transaction_id) {
          const isBatal = salesReturn.reason?.includes('Batal Transaksi') || salesReturn.reason?.includes('Retur Penuh');
          const { data: trx } = await supabase.from('transactions').select('payment_status, remaining_balance, owner_id').eq('id', salesReturn.transaction_id).single();
          ...
          if (trx) {
            ...
            if ((trx.payment_status === 'unpaid' || trx.payment_status === 'partial') && trx.remaining_balance > 0) {
              let newBalance = trx.remaining_balance - (salesReturn.total_refund || 0);
              if (isBatal) {
                 newBalance = 0; // Wipe it completely
              } else {
                 newBalance = Math.max(0, newBalance);
            ...
            if (Object.keys(updatePayload).length > 0) {
              await supabase.from('transactions').update(updatePayload).eq('id', salesReturn.transaction_id);
            }
            ...
            if (debtCancelled > 0) {
              await supabase.from('transaction_payments').insert({
                ...
                amount: debtCancelled,
                payment_method: 'Potongan Retur',
                cashier_name: salesReturn.cashier_name || 'Admin',
                notes: 'Otomatis dari konfirmasi retur',
                payment_date: new Date().toISOString(),
                status: 'confirmed',
                owner_id: trx.owner_id
              });
